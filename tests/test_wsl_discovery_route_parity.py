...
$null = . '{SCRIPT}'
$cases = @(
    @{{ value = (Get-WslRouteImplementationAllowed -StoreServicePresent $true  -InboxServicePresent $false -InboxPolicyAllowed $false) }},
    @{{ value = (Get-WslRouteImplementationAllowed -StoreServicePresent $false -InboxServicePresent $true  -InboxPolicyAllowed $false) }},
    @{{ value = (Get-WslRouteImplementationAllowed -StoreServicePresent $false -InboxServicePresent $true  -InboxPolicyAllowed $true) }},
    @{{ value = (Get-WslRouteImplementationAllowed -StoreServicePresent $false -InboxServicePresent $false -InboxPolicyAllowed $true) }},
    @{{ value = (Get-WslRouteImplementationAllowed -StoreServicePresent $null  -InboxServicePresent $true  -InboxPolicyAllowed $true) }},
    @{{ value = (Get-WslRouteImplementationAllowed -StoreServicePresent $false -InboxServicePresent $null  -InboxPolicyAllowed $true) }}
)
...
'''
    ...
    assert [case["value"] for case in payload] == [True, False, True, False, None, None]
