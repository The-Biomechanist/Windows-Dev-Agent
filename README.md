...
Plan-only calls use `execute: false`. Mutation calls use `execute: true`. For the three plan-first executors, the plan returns an opaque `plan_fingerprint` that must be carried unchanged into execution. The fingerprint is only a freshness/identity guard over the reviewed route/executable/argv/project state; it is not authorization and never substitutes for the host permission boundary. The shared runtime independently blocks capabilities classified `forbidden`; it does not pretend to prove that the host prompt occurred.
...
The shipped PowerShell producer uses live-image optional-feature queries (`Get-WindowsOptionalFeature -Online`). Probe failures are recorded in `snapshot.errors` and make the snapshot degraded rather than silently becoming `false`. Windows Sandbox is queried using its canonical `Containers-DisposableClientVM` optional-feature identity. If environment discovery itself fails before producing a normal snapshot, `env_inspect` still returns that same canonical snapshot shape with unresolved availability left `null`; it does not switch to a second PATH-based truth model.
...
WSL enters the active project using `wsl --cd <project>` and uses `sh -lc` by default. Dev Container execution uses the project configuration and `sh -lc`. Sandbox plans bind the selected environment, resolved backend executable, project, payload paths, launch argv, and inner command into the same freshness guard; a changed backend requires a fresh plan instead of silent substitution.
