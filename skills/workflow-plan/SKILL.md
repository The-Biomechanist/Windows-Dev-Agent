...
4. **Resolve the route.** Read `route_state` before treating a candidate as selected. Deterministic selection requires discriminating overlap with capability identity/tags; generic operation words or description-only similarity are not enough. `selected` means one strongest semantically distinguished route is executable. `ambiguous` means equal strongest task evidence remains; backend/tool availability does not resolve that semantic tie. `unavailable` means the uniquely strongest route lacks an available configured tool; do not silently fall through to a weaker match. `no_match` means the scaffold lacks sufficient discriminating capability evidence and ordinary task reasoning must choose the path. If materially different mechanisms remain live, name the discriminator; do not manufacture cosmetic alternatives.
