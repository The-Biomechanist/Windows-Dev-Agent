...
4. **Resolve the route.** Read `route_state` before treating a candidate as selected. `selected` means one strongest supported match is executable. `ambiguous` means equal top matches remain and the task needs a discriminator. `unavailable` means the strongest match lacks an available configured tool; do not silently fall through to a weaker semantic match. `no_match` means the scaffold has no positive capability evidence and ordinary task reasoning must choose the path. If materially different mechanisms remain live, name the discriminator; do not manufacture cosmetic alternatives.
