...
8. Windows Dev Agent owns Sandbox bundle cleanup. It removes a launched bundle after the Sandbox process exits when that lifecycle is observable and removes stale WDA-owned bundles on later Sandbox startup. Do not ask the user/model to manage a host cleanup path.
