...
4. For Windows Sandbox, identify exactly which project files/directories the isolated command needs and pass them as workspace-relative `payload_paths`. The runtime validates that every selected path stays inside the workspace, rejects symbolic links and Windows reparse-point boundaries such as junctions before traversal crosses them, rejects overlapping selections, and limits staged input to 10,000 filesystem entries and 1 GiB total file bytes. Write the inner command relative to the staged payload root (`C:\WDAShare\payload`).
